...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `StarlarkMap.swiftmodule/`
...
//      `import StarlarkMap` above would fail at compile time.
//
//   2. The static archive `libStarlarkMap.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "StarlarkMap" }` and
//      `flattenPackage = "io.github.kotlinmania.starlarkmap"` configuration in
...
//      via the `StarlarkMapLibrary` product.
//
...
final class StarlarkMapExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "StarlarkMap swift module imported cleanly")
    }
}
